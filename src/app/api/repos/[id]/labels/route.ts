/**
 * @module /api/repos/[id]/labels
 *
 * Fetches labels from the provider (GitHub/GitLab) for a connected repository.
 *
 * - **Endpoint:** `/api/repos/:id/labels`
 * - **HTTP Methods:** GET
...
/**
 * Retrieves all labels for the given repo from its provider API.
 * Returns an empty array if the provider does not support label fetching.
 *
 * @param _req - Incoming request (unused)
 * @param params - Dynamic route params containing `id` (the repo's DB primary key)
 * @returns JSON array of label objects from the provider, or an error with 401/404 status
 */
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const labels = await provider.fetchLabels(repo.owner, repo.name, token);
